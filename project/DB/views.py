...
def custom_signup(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'error': '허용되지 않은 방식의 요청입니다.'}, status=405)
...
def custom_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return JsonResponse({'success': False, 'error': '유효하지 않은 ID입니다.'}, status=400)

    return JsonResponse({'success': False, 'error': '허용되지 않은 방식의 요청입니다.'}, status=405)
...
def save_word(request):
    if request.method == 'POST':
        ...
        if created:
            print("created: True")
            word_obj.save()
            return JsonResponse({'success': True, 'message': f"단어 '{word}'를 데이터베이스에 저장했습니다."}, status=200)
        else:
...
def count_word(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'success': True, 'count_all': count_all, 'count_today': count_today, 'not_learned': not_learned}, status=200)