...
def twoChatbotStory():
    ...
    if token:
        ...
        main_character = file_path + 'Alice2.png'
        llm_character = file_path + 'Alice3.png'
        goal1_image_path = file_path + "Alice7.png"
        goal2_image_path = file_path + "Alice5.png"
        ...
        def word_translator(run_highlighter):
            ...
            for material in material_list:
                material_result = translator.translate(material, src='en', tgt='kr')
                material_result_list.append(material_result)
                # material에서 word를, material_result_list에서 meaning을 가져와 DB에 저장
                response = requests.post(url + app_url + '/save_word/', data={'word': material, 'meaning': material_result, 'userid': localS.getItem('userid')}).json()
                if response['success']:
        ...
        with st.sidebar:
            ...
            with st.container():
                ...
                run_highlighter = st.checkbox("모르는 단어를 저장해보세요!")
